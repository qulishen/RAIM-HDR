...
class Uformer(nn.Module):
    def __init__(self, img_size=256, in_chans=3, dd_in=15,pretrained=None,
        ...
        if pretrained:
            ...
            self.load_state_dict(weights["params"])
            print("loaded pretrained weights from {}".format(pretrained))
...
if __name__ == "__main__":
    model_restoration = Uformer(dd_in=15,depths=[1,2,2,2,2,2,2,2,1])

    # weights = torch.load('/mnt/D/Code/RAIM-HDR/Uformer_B.pth')['state_dict']
    # new_weights = {}
    # for k, v in weights.items():
    #     new_weights[k.replace("module.", "")] = v
    # new_weights['input_proj.proj.0.weight'] = torch.tensor(new_weights['input_proj.proj.0.weight']).repeat((1,5,1,1))
    #
    # model_restoration.load_state_dict(new_weights)

    # from ptflops import get_model_complexity_info
    # macs, params = get_model_complexity_info(model_restoration, (15, 128,128), as_strings=True,
    #                                             print_per_layer_stat=True, verbose=True)
    # print('{:<30}  {:<8}'.format('Computational complexity: ', macs))
    # print('{:<30}  {:<8}'.format('Number of parameters: ', params))
    # print("number of GFLOPs: %.2f G" % (model_restoration.flops() / 1e9))

    x = torch.tensor(torch.randn(1, 15, 512,512))
