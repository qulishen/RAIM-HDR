...
class MST_Plus_Plus(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, n_feat=30, stage=1,pretrained=None):
...
if __name__ == '__main__':
    ...
    model = MST_Plus_Plus(in_channels=18,stage=3,pretrained="mstpp_57.33_net_g_49000.pth")

    model = model.cuda()

    x = torch.tensor(torch.randn(1, 18, 512, 512)).cuda()
    y = model(x)
    print(y.shape)
