...
class MoCEIR(nn.Module):
    def __init__(self,
        ...
        # weights = torch.load("moce_lr_s_aio5.ckpt")['state_dict']
        # new_weights = {}
        # for k,v in weights.items():
        #     new_weights[k.replace('net.', '')] = v
        # new_weights["patch_embed.proj.weight"] = new_weights['patch_embed.proj.weight'].repeat(
        #     (1, 5, 1, 1))
...
if __name__ == "__main__":
    # test
    model = MoCEIR(dim=16,inp_channels=18).cuda()

    x = torch.randn(1, 18, 512,512).cuda()
    _ = model(x)
