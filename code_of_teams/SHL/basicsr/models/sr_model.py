...
class SRModel(BaseModel):
    ...
    def init_training_settings(self):
        ...
        if train_opt.get('npr_opt'):
            self.cri_npr = build_loss(train_opt['npr_opt']).to(self.device)
        else:
            self.cri_npr = None
    ...
    def optimize_parameters(self, current_iter):
        ...
        # perceptual loss
        if self.cri_npr:
            l_npr = self.cri_npr(self.output, self.gt)
            loss_dict['l_npr'] = l_npr
            l_total += l_npr

        try:
            l_total = l_total + self.net_g.total_loss * self.opt.get('model_loss_weight', 1.0)
            loss_dict['l_model_loss'] = self.net_g.total_loss * self.opt.get('model_loss_weight', 1.0)
        except:
            l_total = l_total
