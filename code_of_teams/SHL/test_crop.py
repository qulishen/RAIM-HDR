...
from basicsr.archs.MST_Plus_Plus_arch import MST_Plus_Plus
...
def load_sequence_tensor(seq_dir,crop_size):
    ...
    stacked = torch.from_numpy(np.concatenate(frames, axis=-1)).permute(2, 0, 1).unsqueeze(0)
    full_view_img = frames[2]

    return stacked,full_view_img,ordered_paths
...
def crop_inference(model, inp, full_view_img ,factor=16, block_size=512, shave=32, win_size=8, pad_multiple=None):
    ...
    for y0 in y_starts:
        for x0 in x_starts:
            ...
            patch_extra = cv2.resize(full_view_img, (patch.shape[-2], patch.shape[-2]))
            patch_extra = torch.from_numpy(patch_extra).permute(2, 0, 1).unsqueeze(0)
            patch_extra = patch_extra.to(device)
            patch = torch.cat((patch, patch_extra), dim=1)
...
def build_model(opt_path: str, weights_path: str, device: torch.device):
    ...
    model_name = net_cfg.pop("type", None)
    # model = Restormer(**net_cfg)
    model = eval(model_name)(**net_cfg)
    # model = MoCEIR(**net_cfg)
...
def main():
    ...
    for seq_dir in tqdm(seq_dirs, desc="Infer"):
        inp,full_view_img, _ = load_sequence_tensor(seq_dir,args.crop_size)
        inp = inp.to(device)
        ...
        if args.save_suffix == "jpg":
            save_path = os.path.join(args.output_dir, f"{os.path.basename(seq_dir)}.jpg")
