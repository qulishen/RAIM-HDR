...
def init_net(net, gpu_ids=[], device=None, dist=False, init_type='normal', init_gain=0.02):
    if len(gpu_ids) > 0:
        ...
        if dist:
            # In distributed mode, use the current device set by init_dist
            current_device = torch.cuda.current_device()
            net.to(current_device)
            # Synchronize all processes before DDP creation
            torch.distributed.barrier()
            net = DistributedDataParallel(net, device_ids=[current_device])
        else:
            net.to(device)
            return net
