...
class TimeDiffiT_ResNet_color_128(nn.Module):
    def __init__(
        ...
        input_channels = getattr(dim, 'in_channels', 3) if hasattr(dim, 'in_channels') else 3
        out_channels = 3
        ...
        # Project skip connection when input channels != output channels
        if input_channels != out_channels:
            self.skip_proj = nn.Conv2d(input_channels, out_channels, 1, bias=False)
        else:
            self.skip_proj = None
    ...
    def forward(self, x1, time):
        ...
        if self.skip_proj is not None:
            return self.skip_proj(x1) + self.final_conv(x)
        return x1 + self.final_conv(x)
