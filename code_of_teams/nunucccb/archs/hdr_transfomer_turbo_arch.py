...
class FastFusion(nn.Module):
    """
    极速多帧融合模块
    利用 1x1 降维 + 深度可分离卷积，极大降低 FLOPs 和显存读写延迟
    """

    def __init__(self, in_chans=15, embed_dim=64):
        super().__init__()
        self.fusion = nn.Sequential(
            # 1. 跨帧通道压缩 (15通道 -> embed_dim)，瞬间降维
            nn.Conv2d(in_chans, embed_dim, kernel_size=1, stride=1, padding=0),
            nn.LeakyReLU(0.1, inplace=True),

            # 2. 深度可分离卷积提取空间特征 (groups=embed_dim，速度极快，省显存)
...
# =========================================================================
# 🌟 极速版主网络：HDRTransformerTurbo
# =========================================================================
@ARCH_REGISTRY.register()
class HDRTransformerTurbo(nn.Module):
    def __init__(self, img_size=128, patch_size=1, in_chans=3,
                 ...
                 **kwargs):
        super(HDRTransformerTurbo, self).__init__()

        num_out_ch = 3
        # 假设输入 5 帧图像，每帧 in_chans (3) 个通道，拼接后是 15
        total_in_chans = in_chans * 5

        # --- 1. 极速早期融合 (取代 DCN 和笨重的残差块) ---
        self.early_fusion = FastFusion(in_chans=total_in_chans, embed_dim=embed_dim)

        # --- 2. 极速提取参考帧 (用于最后的全局残差跳连) ---
        self.ref_proj = nn.Conv2d(in_chans, embed_dim, 3, 1, 1)
    ...
    def forward(self, x):
        # 1. 提取中间第 3 帧作为参考帧 (索引 6 到 9 是 3 个通道)
        x_ref = x[:, 6:9, :, :]
        f_ref = self.ref_proj(x_ref)

        # 2. 极速融合所有帧的特征
        x_fused = self.early_fusion(x)

        # 3. Transformer 重建
        res = self.conv_after_body(self.forward_features(x_fused) + x_fused)

        # 4. 加上参考帧的残差，输出
        out = self.conv_last(f_ref + res)
