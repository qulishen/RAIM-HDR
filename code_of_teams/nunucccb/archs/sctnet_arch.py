...
class SCAB(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor=2, bias=False, LayerNorm_type="WithBias"):
        ...
        self.ffn = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=nn.GELU, drop=0.0)
...
class GSAB(nn.Module):
    """Residual Swin Transformer block group with conv residual."""
...
class Upsample(nn.Sequential):
    """Upsample module supporting 2^n or 3 scales."""
...
class UpsampleOneStep(nn.Sequential):
    """Lightweight upsample layer (single conv)."""
...
class SCTNet(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        c_num_heads = [4, 4, 4, 4]
        for i_layer in range(self.num_layers):
            layer = SCAB(embed_dim // 3, num_heads=c_num_heads[i_layer], ffn_expansion_factor=4, bias=False, LayerNorm_type="WithBias")
...
if __name__ == "__main__":
    ...
    model = SCTNet(in_chans=9,img_size=(256, 256)).cuda()
    flops, params = profile(model, inputs=(torch.randn((1, 9, 256, 256)).cuda(),), verbose=False)
