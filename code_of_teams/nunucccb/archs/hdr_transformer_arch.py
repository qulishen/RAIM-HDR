...
# 【关键】：添加装饰器，将其注册到 BasicSR 的网络结构中
@ARCH_REGISTRY.register()
class HDRTransformer(nn.Module):
    def __init__(self, img_size=128, patch_size=1, in_chans=3,  # 注意：这里 in_chans 设为单张图的通道数 3
        ...
        self.conv_f5 = nn.Conv2d(num_in_ch, embed_dim, 3, 1, 1)

        self.att_module_1 = SpatialAttentionModule(embed_dim)
        self.att_module_2 = SpatialAttentionModule(embed_dim)
        self.att_module_4 = SpatialAttentionModule(embed_dim)
        self.att_module_5 = SpatialAttentionModule(embed_dim)

        self.conv_first = nn.Conv2d(embed_dim * 5, embed_dim, 3, 1, 1)
    ...
    def forward(self, x):
        # 将 [B, 15, H, W] 的张量按通道维度均分为 5 份，每份 [B, 3, H, W]
        x1, x2, x3, x4, x5 = torch.chunk(x, chunks=5, dim=1)

        # feature extraction network
        f1 = self.conv_f1(x1)
        f2 = self.conv_f2(x2)
        f3 = self.conv_f3(x3)  # 中间参考帧
        f4 = self.conv_f4(x4)
        f5 = self.conv_f5(x5)

        # spatial feature attention
        f1_att = f1 * self.att_module_1(f1, f3)
        f2_att = f2 * self.att_module_2(f2, f3)
        f4_att = f4 * self.att_module_4(f4, f3)
        f5_att = f5 * self.att_module_5(f5, f3)

        # 将对齐后的 5 个特征在通道维度拼接
        x_fused = self.conv_first(torch.cat((f1_att, f2_att, f3, f4_att, f5_att), dim=1))
    ...
    def flops(self):
        ...
        flops += H * W * 5 * self.embed_dim * 9
        flops += self.patch_embed.flops()
        for i, layer in enumerate(self.layers):
            flops += layer.flops()
        flops += H * W * 5 * self.embed_dim * self.embed_dim
        return flops